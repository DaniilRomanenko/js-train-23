...
class Group {
  ...
  // Має початкове значення: пустий об'єкт.
  static #groups = {};

  // Об'єкт використовується для зберігання груп, де ключ - це назва групи, а значення - екземпляр групи.
  #groups = {};

  // Конструктор класу, який приймає назву групи як аргумент та ініціалізує поле this.name.
  constructor(name) {
    this.name = name;
  }

  // Статичний метод create, який приймає назву групи name як аргумент.
  static create(name) {
    // Перевірка чи група з такою назвою ще не була створена.
    if (!this.#groups[name]) {
      // Якщо група ще не існує, створюємо її та зберігаємо в полі #groups.
...
class Product {
  // Конструктор класу, який приймає назву продукту name та групу group як аргументи та ініціалізує відповідні поля.
  constructor(name, group) {
    this.name = name;
    this.group = group;
  }

  // Метод display, який виводить інформацію про продукт в консоль.
  display() {
